						...
						<div class="card-body">
							<form id="manage-filter" action="index.php?page=flights" method="POST">
								<div class="row form-group">
									...
									<div class="col-sm-3">
										<label for="" class="control-label">Departing On</label>
									...
									<div class="col-sm-3" id="rdate" <?php if (isset($trip) && $trip == 1) : ?> style="display: none" <?php endif; ?>>
										<label for="" class="control-label">Returning On</label>
								...
								<div class="row form-group">
									<div class="col-sm-2 text-center">
										<div class="form-check">
											<input class="form-check-input" type="radio" name="trip" id="trip" value="1" <?php echo isset($trip) && $trip == 1 ? "checked" : "" ?>>
									...
									<div class="col-sm-2 text-center">
										<div class="form-check">
											<input class="form-check-input" type="radio" name="trip" id="trip" value="2" <?php echo isset($trip) && $trip == 2 ? "checked" : "" ?>>
				...
				<div class="col-lg-12">
					<div class="row">
						<div class="col-md-12 text-center">
							<h2><b><?php echo isset($trip) && $trip == 2 ? "Departing & Returning Searched Flight results..." : (!isset($trip) ? " Flights Available " : "Searched Flight results...")  ?></b></h2>
						</div>
					</div>
					<hr class="divider">
					<?php
					...
					?>
							...
							<div class="row align-items-center">
								...
								<div class="col-md-6">
									...
									<p><small>Departing On: <b><?php echo date('M d,Y h:i A', strtotime($row['departure_datetime'])) ?></b></small></p>
									<?php
									if(isset($trip) && $trip == 2){
									?>
									<p><small>Returning On: <b><?php echo (date('M d,Y', strtotime($row['departure_datetime'])) == date('M d,Y', strtotime($row['arrival_datetime']))) ? date('h:i A', strtotime($row['arrival_datetime'])) : date('M d,Y h:i A', strtotime($row['arrival_datetime'])) ?></b></small></p>
									<?php }?>
									<p>Available Seats : <b>
								...
								<div class="col-md-3 text-center align-self-end-sm">
									...
									$_SESSION['trip'] = $trip;
									?>
									<button class="btn-outline-primary  btn  mb-4 book_flight" type="button" data-id="<?php echo $row['id'] ?>" data-name="<?php echo $aname[$row['departure_airport_id']] . ' - ' . $aname[$row['arrival_airport_id']] ?>" data-max="<?php echo $row['seats'] - $booked; ?>" data-trip="<?php echo $trip ?>">Book Now</button>
								</div>
							</div>
