				...
				<table class="table table-bordered" id="flight-list">
					...
					<thead>
						<tr>
							...
							<th class="text-center">Flight Information</th>
						...
						?>
							<tr>
								...
								<td>
									...
									<p><small>Trip Type:<b><?php if ($row['trip'] == 2) {
																echo "Round Trip";
															} else {
																echo "One Way";
															} ?></small></b></p>
								...
								<td>
									<div class="row">
										...
										<div class="col-sm-6">
											...
											<p><small>Payment :<b><?php echo $row["price"] ?></small></b></p>
											<p><small>Location :<b><?php echo $aname[$row['departure_airport_id']] . ' - ' . $aname[$row['arrival_airport_id']] ?></small></b></p>
											<p><small>Departuring On :<b><?php echo date('M d,Y h:i A', strtotime($row['departure_datetime'])) ?></small></b></p>
											<?php if ($row['trip'] == 2) { ?>
												<p><small>Returning On :<b><?php echo date('M d,Y h:i A', strtotime($row['arrival_datetime'])) ?></small></b></p>
											<?php } ?>
										</div>
									</div>
								</td>
	...
	function delete_booked($id) {
		...
		$.ajax({
			...
			success: function(resp) {
				console.log(resp);
				if (resp == 1) {
					alert_toast("Book Flight successfully deleted", 'success')
					setTimeout(function() {
						location.reload()
					}, 1500)
